...
Page({
    ...
    addNewMsg(content, sendStatus) {
        ...
        var newMsg = {
            ...
            isMy: true,
            headUrl: app.globalData.userInfo.userInfo.headImgs,
            content: content,
        ...
        wx.chooseImage({
            ...
            success: (res) => {
                ...
                _this.uploadFile(res.tempFilePaths[0],(result)=>{
                    ...
                    lastMsg.forEach(item => {
                        if (item.fromAccount == app.chatData.toUser.id) {
                            item.lastMsg = _this.data.langData.otherText[_this.data.lang]
                            isHasfromAccount = true;
                        }
                    });
                    if (!isHasfromAccount) {
                        lastMsg.push({
                            fromAccount: app.chatData.toUser.id,
                            lastMsg: _this.data.langData.otherText[_this.data.lang]
                        })
                    }
                    wx.setStorageSync('lastMsg', lastMsg);
